...
const DEFAULT_CONTENT = {
  ...
  events: [
    { id: "evt_1", theme: "The Summer Edit", date: "April 18, 2026", day: "Saturday, April 18", timing: "10 AM – 6 PM", location: "Mindspace, Madhapur", fullLocation: "Raheja Mindspace · Madhapur, Hyderabad", stalls: "40+ Curated Spots", entry: "Free · All Welcome", status: "upcoming" },
    { id: "evt_2", theme: "The Monsoon Edit", date: "May 16, 2026", day: "Saturday, May 16", timing: "10 AM – 6 PM", location: "DLF Cybercity, Gachibowli", fullLocation: "DLF Cybercity · Gachibowli, Hyderabad", stalls: "40+ Curated Spots", entry: "Free · All Welcome", status: "upcoming" },
    { id: "evt_3", theme: "The Craft Mela", date: "June 20, 2026", day: "Saturday, June 20", timing: "10 AM – 6 PM", location: "Ascendas IT Park, Hitec City", fullLocation: "Ascendas IT Park · Hitec City, Hyderabad", stalls: "50+ Curated Spots", entry: "Free · All Welcome", status: "upcoming" },
    { id: "evt_4", theme: "The Heritage Edit", date: "July 18, 2026", day: "Saturday, July 18", timing: "10 AM – 6 PM", location: "RMZ Futura, Kadubeesanahalli", fullLocation: "RMZ Futura · Kadubeesanahalli, Hyderabad", stalls: "45+ Curated Spots", entry: "Free · All Welcome", status: "upcoming" },
  ],
...
const DEMO_SUBMISSIONS = [
  { id: "sub_001", name: "Priya Mehta", biz: "Wildbloom Studio", cat: "Jewelry & Accessories", insta: "@wildbloom.studio", phone: "+91 98400 11234", msg: "We make handcrafted silver and semi-precious stone jewelry. Each piece is one of a kind.", events: ["The Summer Edit — Apr 18, Raheja Mindspace", "The Monsoon Edit — May 16, DLF Cybercity"], stall: "Standard 8x6 ft — ₹2,500/day", status: "pending", date: "1 Mar 2025" },
  { id: "sub_002", name: "Aditya Rao", biz: "Ferment & Co.", cat: "Artisanal Food", insta: "@fermentandco", phone: "+91 87654 32100", msg: "Artisanal kombuchas, fermented hot sauces and kimchi. We've been selling at HITEC area offices for 6 months.", events: ["The Summer Edit — Apr 18, Raheja Mindspace"], stall: "Food Stall — ₹5,000/day", status: "approved", date: "3 Mar 2025" },
  { id: "sub_003", name: "Sneha Krishnan", biz: "Earthen Home", cat: "Home & Décor", insta: "@earthenhome.hyd", phone: "+91 99887 76655", msg: "Handmade terracotta pots, hand-painted ceramics and macramé wall hangings.", events: ["The Craft Mela — Jun 20, Ascendas IT Park", "The Heritage Edit — Jul 18, RMZ Futura"], stall: "Basic 6x4 ft — ₹1,500/day", status: "approved", date: "5 Mar 2025" },
  { id: "sub_004", name: "Rahul Verma", biz: "The Print Room", cat: "Art & Prints", insta: "@theprintroom_hyd", phone: "+91 77665 54433", msg: "Limited edition giclée prints of Hyderabad cityscapes and abstract works. All archival quality.", events: ["The Heritage Edit — Jul 18, RMZ Futura"], stall: "Standard 8x6 ft — ₹2,500/day", status: "pending", date: "6 Mar 2025" },
  { id: "sub_005", name: "Lakshmi Iyer", biz: "Petal & Pot", cat: "Plants & Botanicals", insta: "@petalandpot", phone: "+91 90001 23456", msg: "Rare succulents, air plants, custom terrariums and handmade planters.", events: ["The Monsoon Edit — May 16, DLF Cybercity"], stall: "Basic 6x4 ft — ₹1,500/day", status: "rejected", date: "28 Feb 2025" },
  { id: "sub_006", name: "Karthik Nair", biz: "Thali & Spice", cat: "Artisanal Food", insta: "@thaliandspice", phone: "+91 81234 56789", msg: "Traditional Hyderabadi spice blends, pickles and homemade mithai.", events: ["The Summer Edit — Apr 18, Raheja Mindspace", "The Craft Mela — Jun 20, Ascendas IT Park", "The Heritage Edit — Jul 18, RMZ Futura"], stall: "Food Stall — ₹5,000/day", status: "pending", date: "7 Mar 2025" },
];
              ...
              <table>
                ...
                <tbody>
                  {recent.map(s => (
                    <tr key={s.id}>
                      <td>
                        <div className="td-biz">
                        {s.biz}
                        {s.events && s.events.length > 1 && <span style={{fontSize:'.6rem',background:'rgba(193,122,90,.15)',color:'#c17a5a',padding:'1px 6px',marginLeft:6,borderRadius:2}}>{s.events.length} events</span>}
                      </div>
                        <div className="td-meta">{s.name}</div>
                      </td>
                      <td><div className="td-meta">{s.cat}</div></td>
                      <td><div className="td-time">{fmtShort(s.date)}</div></td>
            ...
            <div className="panel-body">
              <div className="quick-actions">
                ...
                <div className="quick-card" onClick={() => onNav('edition')}>
                  ...
                  <div><div className="quick-label">Edition Details</div><div className="quick-sub">{(content.events||[])[0]?.date || 'No events'}</div></div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="panel-body" style={{paddingTop:8, paddingBottom:8}}>
              {recent.slice(0,4).map((s,i) => (
                <div className="activity-item" key={s.id}>
                  ...
                  <div>
                    <div className="activity-text"><strong style={{color:'var(--cream)'}}>{s.biz}</strong> applied as vendor</div>
                    <div className="activity-time">{fmtShort(s.date)}</div>
                  </div>
                </div>
              ))}
            </div>
...
function SubmissionsTab({ submissions, onUpdate, showToast }) {
  ...
  const filtered = submissions.filter(s => {
    ...
    const matchSearch = !q || s.name.toLowerCase().includes(q) || s.biz.toLowerCase().includes(q) || s.cat.toLowerCase().includes(q);
        ...
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setSelected(null)}>
          <div className="modal">
            <div className="modal-header">
              <div className="modal-title">{selected.biz}</div>
            ...
            <div className="modal-body">
              ...
              {[['Applicant Name', selected.name],['Business Name',selected.biz],['Category',selected.cat],['Stall Size',selected.stall||'—'],['Instagram',selected.insta],['Phone',selected.phone],['Applied On',selected.date]].map(([k,v])=>(
              ...
              <div className="modal-field">
                ...
                <div className="modal-field-val">{selected.msg}</div>
              </div>
              {selected.events && selected.events.length > 0 && (
                <div className="modal-field">
                  <div className="modal-field-key">Applied For ({selected.events.length} edition{selected.events.length > 1 ? 's' : ''})</div>
                  <div style={{display:'flex',flexDirection:'column',gap:6,marginTop:4}}>
                    {selected.events.map((ev,i) => (
                      <div key={i} style={{fontSize:'.8rem',color:'var(--cream)',background:'rgba(193,122,90,.1)',border:'1px solid rgba(193,122,90,.2)',padding:'5px 12px',display:'flex',alignItems:'center',gap:8}}>
                        <span style={{color:'var(--gold)',fontSize:'.7rem'}}>◆</span> {ev}
                      </div>
                    ))}
                  </div>
                </div>
...
function EditionTab({ content, onChange, showToast }) {
  ...
  useEffect(() => setEvents(content.events || []), [content.events]);

  const STATUS_COLORS = { upcoming: '#c17a5a', live: '#4caf50', past: '#888' };

  const startEdit = (evt) => { setEditing(evt.id); setEditData({ ...evt }); };
  const cancelEdit = () => { setEditing(null); setEditData({}); };

  const saveEvent = async () => {
    setSaving(true);
    const updated = events.map(e => e.id === editing ? { ...editData } : e);
    setEvents(updated);
    await onChange({ ...content, events: updated });
    setTimeout(() => { setSaving(false); showToast('Event saved!'); setEditing(null); }, 500);
  };

  const addEvent = async () => {
    const newEvt = { id: 'evt_' + Date.now(), theme: 'New Edition', date: '', day: '', timing: '10 AM – 6 PM', location: '', fullLocation: '', stalls: '40+ Curated Spots', entry: 'Free · All Welcome', status: 'upcoming' };
    const updated = [...events, newEvt];
    setEvents(updated);
    await onChange({ ...content, events: updated });
    startEdit(newEvt);
    showToast('New event created — fill in the details below');
  };
  ...
  const fields = [
    ['theme', 'Edition Name', 'e.g. The Summer Edit'],
    ['date', 'Display Date', 'e.g. April 18, 2026'],
    ['day', 'Day (short)', 'e.g. Saturday, April 18'],
    ['timing', 'Timings', 'e.g. 10 AM – 6 PM'],
    ['location', 'Location (short)', 'e.g. Mindspace, Madhapur'],
    ['fullLocation', 'Full Location', 'e.g. Raheja Mindspace · Madhapur, Hyderabad'],
    ['stalls', 'Stalls Available', 'e.g. 40+ Curated Spots'],
  ...
  return (
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:20 }}>
        <h2 style={{ fontSize:'1.1rem', fontWeight:400, color:'#f0ebe3' }}>Events Manager</h2>
        <button className="save-btn" onClick={addEvent} style={{ display:'flex', alignItems:'center', gap:6 }}>+ Add New Event</button>
      </div>

      {events.map(evt => (
        <div key={evt.id} className="panel" style={{ marginBottom:16 }}>
          {editing === evt.id ? (
            <>
              <div className="panel-header" style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <span className="panel-title">Editing: {editData.theme || 'New Event'}</span>
                <div style={{ display:'flex', gap:8 }}>
                  <select value={editData.status||'upcoming'} onChange={e => setEditData(p=>({...p,status:e.target.value}))}
                    style={{ background:'#1e1a14', border:'1px solid #3a3428', color:'#c8b99a', padding:'4px 10px', fontSize:'.75rem', borderRadius:2 }}>
                ...
                <div className="grid-2">
                  {fields.map(([key, label, placeholder]) => (
                    <div className="field-group" key={key}>
                      ...
                      <input className="field-input" value={editData[key]||''} onChange={e => setEditData(p=>({...p,[key]:e.target.value}))} placeholder={placeholder} />
                    </div>
                  ))}
                </div>
              </div>
              <div style={{ padding:'16px 24px', display:'flex', gap:10, borderTop:'1px solid #2a2520' }}>
                <button className={`save-btn ${saving?'success':''}`} onClick={saveEvent}><Icon.Save />{saving?'Saved!':'Save Event'}</button>
            ...
            <>
              <div className="panel-header" style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                  <span className="panel-title">{evt.theme}</span>
                  <span style={{ fontSize:'.65rem', letterSpacing:'.2em', textTransform:'uppercase', color: STATUS_COLORS[evt.status] || '#c17a5a', background:'rgba(193,122,90,.1)', padding:'2px 8px', borderRadius:2 }}>{evt.status}</span>
                </div>
                <div style={{ display:'flex', gap:8 }}>
                  <button onClick={() => startEdit(evt)} style={{ background:'transparent', border:'1px solid #3a3428', color:'#c8b99a', padding:'5px 12px', fontSize:'.72rem', cursor:'pointer', display:'flex', alignItems:'center', gap:5, letterSpacing:'.1em' }}><Icon.Edit /> Edit</button>
                  <button onClick={() => deleteEvent(evt.id)} style={{ background:'transparent', border:'1px solid #5a2020', color:'#c07070', padding:'5px 10px', fontSize:'.72rem', cursor:'pointer' }}><Icon.Trash /></button>
                </div>
              </div>
              <div className="panel-body" style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:'12px 24px' }}>
                {[['Date', evt.date], ['Location', evt.location], ['Timings', evt.timing], ['Stalls', evt.stalls], ['Entry', evt.entry], ['Full Location', evt.fullLocation]].map(([k,v]) => (
                  <div key={k}>
                    <div style={{ fontSize:'.58rem', letterSpacing:'.25em', textTransform:'uppercase', color:'#c17a5a', marginBottom:3 }}>{k}</div>
                    <div style={{ fontSize:'.82rem', color:'#c8b99a' }}>{v || '—'}</div>
                  </div>
                ))}
              </div>
            </>
